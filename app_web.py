...
# Estilo para emular tu app (Fondo de botones y títulos)
st.markdown("""
    <style>
    .stButton>button {
...
with st.sidebar:
    ...
    if tipo == "Placa (2D)":
        ...
        pw = st.number_input("Ancho Pieza (cm):", value=60.0)
        ph = st.number_input("Alto Pieza (cm):", value=50.0)
        cant = st.number_input("Cantidad total piezas:", value=1, min_value=1, step=1)
    
    elif tipo == "Marco (45°)":
        bl = st.number_input("Largo de la Barra (cm):", value=610.0)
        ml = st.number_input("Largo de la Caja (cm):", value=100.0)
        ma = st.number_input("Ancho de la Caja (cm):", value=80.0)
        offset = st.number_input("Offset p/ 45° (cm):", value=5.0)
        cant_marcos = st.number_input("Número de Cajas:", value=1, min_value=1, step=1)
        
    else: # Tramos 1D
        bl = st.number_input("Largo de la Barra (cm):", value=610.0)
        tl = st.number_input("Medida del tramo (cm):", value=5.0)
...
if st.button("🚀 Calcular Optimización"):
    ...
    if tipo == "Placa (2D)":
        try:
            # Limpieza y forzado de tipos para evitar TypeError
            ancho_h = float(hw)
            alto_h = float(hh)
            ...
            # Usamos nombres de argumentos para máxima compatibilidad
            packer.add_rect(width=ancho_p, height=alto_p, count=cantidad_p)
            packer.pack()
            ...
            if not hojas_usadas:
                ...
                for b_id, rects in hojas_usadas.items():
                    ...
                    # Fondo gris de la hoja (sobrante)
    ...
    # --- MODO MARCO Y TRAMOS (1D) ---
        ...
        try:
            ...
            for i, cortes in enumerate(barras_finales):
                ...
                ax.set_title(f"Barra #{i+1}", color="white", loc="left")
                ax.axis('off')
                st.pyplot(fig)
        except Exception as e:
